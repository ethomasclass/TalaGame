// ---------------- state (from the spec) ----------------
const STATE = { mornings:[], honesty:0, invitedHannah:false, cook:{}, wire:{}, seen:[], errand:{left:30, done:[], vinegar:null} };
...
// ---------------- content ----------------
const NAMES = {tala:'Tala', ma:'Ma', pa:'Pa', customer:'Customer', hannah:'Hannah', ando:'Ando', tita:'Tita Cora', clerk:'The clerk', mangboy:'Mang Boy', ocampo:'Mrs Ocampo'};
const ROLES = {tala:'seventeen · two years here', ma:'Divina Ramos · runs the dining room', pa:'Rey Ramos · kitchen · was a pharmacist for eleven years', customer:'eats here maybe twice a year',
               hannah:'third period · one desk over since September', ando:'cousin · twenty · landed today', tita:'Corazon Villaflor · the parish food committee', clerk:'Newark Avenue · does this two hundred times a week', mangboy:'has had the store since before Tala was born', ocampo:'four doors down · knows everything'};
const met = new Set();
...
const TERMS = {
  ...
  'po':'a small word that makes anything polite', 'niyog':'grated coconut', 'muscovado':'dark, raw sugar', 'malagkit':'sticky rice', 'pirurutong':'the purple rice', 'bumbong':'the bamboo tube', 'kaunti lang':'just a little', 'salabat':'hot ginger tea', 'pan de sal':'soft bread rolls. Every Filipino bakery bakes them twice a day', 'videoke':'karaoke with a machine that scores you. A fixture at any Filipino party', 'sari-sari':'a little bit of everything. What a corner shop like this is called',
...
function gloss(html){ if(!html) return html; let out = html; const stash = [];
  // Longest first, and each match is stashed behind a placeholder so a shorter term can never
  // land inside one already wrapped ("bumbong" inside "puto bumbong") or inside a definition.
  for(const t of Object.keys(TERMS).sort((a,b) => b.length - a.length)){
    const re = new RegExp('(^|[^\\w>])(' + t.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + ')(?![\\w])(?![^<]*>)');
    if(re.test(out)) out = out.replace(re, (m, pre, w) => {
      stash.push(`<span class="term" data-def="${TERMS[t]}">${w}</span>`);
      return `${pre}\u0000${stash.length - 1}\u0000`; }); }
  return out.replace(/\u0000(\d+)\u0000/g, (m, n) => stash[n]); }
...
const HOT = {
  dining:[ {x:132,y:170,t:'The parol. Lola sent it the first December, folded flat in a balikbayan box. The note said it was for the window, not for keeping in the box.'},
           ...
           {x:1168,y:598,t:'The envelope. The remittance receipts go in the drawer under the register. There are a lot of them.'} ],
  rest:[ {x:180,y:220,t:'Same parol as home. Ma says one window is not enough for a whole December.'},
         ...
         {x:252,y:462,t:'Table two. Nobody, all night.'} ],
  street:[ {x:104,y:340,t:'Bayanihan Cargo. You buy an empty box, you fill it for a year, they ship it by sea and it gets there in two months. Chocolate, shoes, medicine, a jacket for somebody who has never needed a jacket.'},
           {x:312,y:338,t:'Mang Boy’s. He is not called Boy. Nobody remembers what he is called. He has run this store since before Tala was born and he lets half the block pay at the end of the month.'},
           {x:652,y:400,t:'The panaderia. Second batch of pan de sal comes out at five, because everybody gets off work at five. That is the whole business plan and it has worked for thirty years.'},
           {x:830,y:330,t:'A teleserye on the salon TV that nobody is watching and nobody will turn off. Tala’s mother watches this one. So does Bea’s mother, six thousand miles away, one day behind.'},
           {x:1115,y:236,t:'Quik Mart. Milk, batteries, lottery tickets. It is cheaper and it is fine and it does not know anybody’s name.'},
           {x:618,y:120,t:'Somebody upstairs has a parol in the window. There are eleven on this block. Tala counted, the first December, when she was still counting things.'} ],
  ...
  wire:[ {x:350,y:104,t:'The rate board. It was 53.10 on Monday. Nobody in this room controls that number, and everybody in this room watches it.'},
         ...
         {x:1184,y:598,t:'The bell. You ring it if nobody comes. Somebody always comes.'},
         {x:1000,y:604,t:'The form. Name, address, relationship to the person receiving. Tala has written “grandmother” in that box eleven times this year.'} ],
  hall:[ {x:1188,y:322,t:'The videoke machine. It belongs to the parish, it is older than most of the nephews operating it, and it has been to every wedding, funeral and birthday on this block.'},
         {x:330,y:360,t:'Salabat. Ginger tea, no milk, hot enough to hurt on the way down. It is what you drink at five in the morning in December after standing outside for an hour.'},
         {x:852,y:378,t:'Pan de sal from the panaderia on Newark Avenue. Somebody paid for eight dozen and has not said who.'},
         {x:660,y:158,t:'Maligayang Pasko. Merry Christmas. The banner is older than the parish’s priest. Tita Cora brings it every year in a garment bag.'},
         {x:1010,y:500,t:'Puto bumbong on banana leaf. Purple rice, steamed standing up. The smell in the cold is the whole point.'},
         {x:1150,y:552,t:'Bibingka. Salted egg and cheese on top, coals above and below. Lola’s always had a burnt spot on one side. On purpose.'} ]
};
let looking = false, zoomOn = false;
// With transform-origin at o and scale k, a point p lands at o + k*(p-o). Solve for the o
// that puts the target where we want it framed, rather than guessing an origin.
const FRAME = [640, 292];   // a little above centre, so the subject clears the dialogue panel
function pushIn(x, y, k){ const el = $('street'); zoomOn = true;
  // The layer's edges map to o*(1-k) and k*W + o*(1-k), so an origin outside [0,W] pulls the
  // artwork off the frame and shows background. Clamp instead: a stop near the edge of the
  // block ends up framed off-centre rather than with a black gap beside it.
  const clamp = (v, hi) => Math.max(0, Math.min(hi, v));
  const ox = clamp((FRAME[0] - k * x) / (1 - k), 1280), oy = clamp((FRAME[1] - k * y) / (1 - k), 720);
  el.style.transformOrigin = `${ox / 1280 * 100}% ${oy / 720 * 100}%`; el.style.transform = `scale(${k})`; }
function pullOut(){ if(!zoomOn) return; zoomOn = false; const el = $('street'); el.style.transform = ''; }
...
function look(text, key){ looking = true;
  ...
  $('who').innerHTML = 'Tala <span class="role">· looking</span>'; $('who-av').className = 'av port av-tala'; $('who-av').hidden = false;
  $('line').className = 'line look'; $('line').innerHTML = gloss(text); $('aside').hidden = true; drawHot(); }
function unlook(){ looking = false; render(); }

// Forty minutes before service, ten of them getting back. Thirty to spend on the block.
// The arithmetic is the mechanic: the neighbourhood store costs more time AND more money
// than the chain, so buying the right vinegar costs you a stop.
const STOPS = {
  mangboy:{ label:'Mang Boy’s. The right vinegar, on the tab.', min:14, goto:'stop-mangboy', vinegar:'cane' },
  quik:   { label:'Quik Mart. Vinegar, four dollars cheaper.',  min:6,  goto:'stop-quik',    vinegar:'chain' },
  pan:    { label:'The panaderia. The second tray is still out.', min:10, goto:'stop-pan' },
  salon:  { label:'The salon. Mrs Ocampo is in the window.',      min:11, goto:'stop-salon' },
  cargo:  { label:'Bayanihan Cargo. Price a box for Lola.',       min:16, goto:'stop-cargo' }
};
function errandChoices(){
  const e = STATE.errand, out = [];
  for(const k in STOPS){ const s = STOPS[k];
    if(e.done.includes(k)) continue;
    if(s.vinegar && e.vinegar) continue;      // she only needs vinegar once
    // always keep back enough time for the cheapest vinegar, so she can never be stranded without it
    const reserve = (!e.vinegar && !s.vinegar) ? STOPS.quik.min : 0;
    if(s.min > e.left - reserve) continue;
    out.push({ text:`${s.label}<span class="mins">${s.min} min</span>`, goto:s.goto,
      set: () => { e.left -= s.min; e.done.push(k);
        if(s.vinegar){ e.vinegar = s.vinegar; STATE.cook = Object.assign({}, STATE.cook, {vinegar:s.vinegar}); } } });
  }
  if(e.vinegar) out.push({ text:'Head back. Pa is waiting on the vinegar.', goto:'errand-done' });
  return out;
}
...
const beats = [
  ...
  {note:['Newark Avenue','Jersey City has one of the largest Filipino communities in the United States, and a lot of it shops on this block: a cargo shop, a grocery, a bakery, a salon, and then a chain store on the corner.',
         'Geographers call a neighbourhood like this an <b>ethnic enclave</b>. Enough people from one country live close enough together that a business can survive selling only to them. Each shop is part of the reason the next one is still open.',
         'Mang Boy’s carries the exact cane vinegar Lola’s recipe needs and lets the family pay at the end of the month. The chain is four dollars cheaper and does not carry it at all.']},
  {hud:['December 18','Third morning · Before service'], bg:'street',
     stage:'Newark Avenue at twenty past five, and the block is louder than the restaurant. The bakery has its second tray of pan de sal out, because everybody gets off work at five. Somebody upstairs is singing along to something and not stopping.',
     aside:'The small lights are things Tala could look at. There is time.'},
  {bg:'street', stage:'She has walked this block a thousand times. It is the one place in New Jersey where nobody has ever asked her where she is from.'},
  {bg:'street', stage:'Pa is out of vinegar and service starts in forty minutes. Ten of those are walking back with it.',
     aside:'Thirty minutes of block. She is not going to get to everything on it.'},

  // ===== the errand. Come back here after every stop until the vinegar is bought. =====
  {id:'errand', bg:'street', hud:['December 18', () => `Third morning · ${STATE.errand.left} min until service`],
     tip: () => `<em>${STATE.errand.left} minutes</em> before Pa needs the vinegar. Where does she go?`,
     choice: () => errandChoices()},

  {id:'stop-mangboy', bg:'street', zoom:[380,400,2.0], hud:['December 18', () => `Third morning · ${STATE.errand.left} min until service`],
     stage:'Eight feet wide, and it smells like fish sauce and laundry soap. There is a radio on behind the register.'},
  {bg:'street', zoom:[380,400,2.0], who:'mangboy', tl:'Kumusta na ang lola mo?', text:'How is your grandmother?',
     aside:'He asked about Lola before he asked what she wanted. He does that every time, and Tala has never once heard him get a name wrong.'},
  {bg:'street', zoom:[380,400,2.0], who:'tala', text:'She is in the hospital again.'},
  {bg:'street', zoom:[380,400,2.0], who:'mangboy', text:'…Tell your mother I said to send my regards. And take the cane vinegar, not the white. Two?'},
  {bg:'street', zoom:[380,400,2.0], stage:'He writes it in a notebook under the family name and does not say when to pay. On the way out he turns the radio up so she can hear it on the street.', goto:'errand'},

  {id:'stop-quik', bg:'street', zoom:[1115,400,1.9], hud:['December 18', () => `Third morning · ${STATE.errand.left} min until service`],
     stage:'Fluorescent, four aisles, a self-checkout with a bag over it. The vinegar is a gallon jug next to the bleach and it is not cane.'},
  {bg:'street', zoom:[1115,400,1.9], stage:'Nobody speaks to her. She is out in six minutes and four dollars up.',
     aside:'Four dollars is four dollars. She will think about this again on Sunday.', goto:'errand'},

  {id:'stop-pan', bg:'street', zoom:[652,410,2.0], hud:['December 18', () => `Third morning · ${STATE.errand.left} min until service`],
     stage:'The second tray is out and the whole shop is warm in a way the street is not. The woman behind the counter is already putting them in paper before Tala has said anything.'},
  {bg:'street', zoom:[652,410,2.0], stage:'Two dozen pan de sal for six dollars, because it is late and they do not keep. Tala buys them without asking anybody.',
     aside:'She will have to explain the six dollars. She is going to say they were free.',
     set: () => { STATE.errand.pandesal = true; }, goto:'errand'},

  {id:'stop-salon', bg:'street', zoom:[830,392,2.0], hud:['December 18', () => `Third morning · ${STATE.errand.left} min until service`],
     stage:'Mrs Ocampo is under the dryer with her eyes shut and the television going. She opens one eye without moving anything else.'},
  {bg:'street', zoom:[830,392,2.0], who:'ocampo', text:'Tala. Your mother is doing the puto bumbong for the parish.'},
  {bg:'street', zoom:[830,392,2.0], who:'tala', text:'She said yes at the hall.'},
  {bg:'street', zoom:[830,392,2.0], who:'ocampo', text:'She said yes because Cora asked her in a room with forty people in it.'},
  {bg:'street', zoom:[830,392,2.0], who:'ocampo', text:'Your mother has never said no in a full room. Not once, in twenty years. Ask her sometime what that has cost her.',
     aside:'Tala has known this woman her whole life and did not know that about her.',
     set: () => { STATE.errand.salon = true; }, goto:'errand'},

  {id:'stop-cargo', bg:'street', zoom:[100,400,2.0], hud:['December 18', () => `Third morning · ${STATE.errand.left} min until service`],
     stage:'The man behind the counter is taping a box shut with one knee on the lid. Behind him is a wall of flattened empty boxes for sale.'},
  {bg:'street', zoom:[100,400,2.0], stage:'A jumbo box, door to door to Batangas, is ninety-five dollars and goes by sea. Two months. He says it like it is fast, and for a box it is.'},
  {bg:'street', zoom:[100,400,2.0], stage:'Tala does the arithmetic in her head and does not buy one.',
     aside:'Ninety-five dollars, landing in February. Whatever Pa ends up wiring will be there the same night, and cost more to send.',
     set: () => { STATE.errand.cargo = true; }, goto:'errand'},

  {id:'errand-done', bg:'street', hud:['December 18','Third morning · Before service'],
     stage: () => {
       const d = STATE.errand.done, bits = [];
       if(d.includes('pan')) bits.push('two dozen pan de sal she is going to say were free');
       if(d.includes('cargo')) bits.push('the price of a box in her head');
       if(d.includes('salon')) bits.push('something about her mother she did not have this morning');
       const carry = bits.length ? ` She is also carrying ${bits.join(', and ')}.` : '';
       return (STATE.errand.vinegar === 'cane'
         ? 'She walks back with the cane vinegar and the notebook entry.'
         : 'She walks back with a gallon jug of white vinegar and four dollars.') + carry;
     }},
  ...
  {hud:['December 18','Third morning · Dinner service'], bg:'kitchen',
     stage: () => 'Ando landed at two o’clock. By six he is tying on an apron.'
       + (STATE.errand.pandesal ? ' Somebody has put two dozen pan de sal on the pass and nobody has admitted to buying them.' : ''),
     expr:{pa:'neutral',ando:'tired'}},
  ...
  {bg:'rest', who:'ma', text:'No. But the restaurant has to open again tomorrow.', expr:{ma:'held'},
     aside: () => STATE.errand.salon
       ? 'Mrs Ocampo said she has never once said no in a full room. This room had one customer in it and she still did not.' : null},
  ...
  {bg:'wire', stage: () => STATE.wire.speed === 'fast'
     ...
     aside: () => STATE.errand.cargo
       ? 'Ninety-five dollars to send a box that lands in February. Fourteen to send money that lands tonight. Tala has now priced both ways of getting something home.' : null,
  ...
  {hud:['December 24','Ninth morning · 5:02 a.m.'], bg:'dawn', show:{'dawn-walk':true,'dawn-tala':false},
     stage:'They push it three blocks to the parish hall on a hand cart.'},
  {hud:['December 24','Ninth morning · The parish hall'], bg:'hall',
     show:{'hall-hannah':false,'hall-basket':false,'hall-food':true},
     stage:'Mass ends at ten to six and the hall fills up in about ninety seconds. Three conversations in Tagalog, one in English, one that switches every other sentence. Somebody’s baby. The urn of salabat hissing at the end of the table.',
     expr:{tala:'quiet',ma:'neutral',tita:'neutral'}},
  {bg:'hall', stage:'Bibingka, puto bumbong, pan de sal, and a tray of something Tala does not recognise that Mrs Ocampo brought anyway. Five in the morning, and there is more food in this room than the restaurant sells on a Saturday night.',
     aside:'Nobody in here is rich. Everybody in here brought something.', expr:{tala:'neutral'}},
  {hud:['December 24','Ninth morning · The parish hall'], bg:'hall',
  ...
  {bg:'hall', stage:'Then a woman Tala has never spoken to puts bibingka on Hannah’s plate. Another one adds pan de sal. A third asks whose friend she is, and does not wait for the answer before adding more.', expr:{hannah:'curious',tala:'smile'}},
  {bg:'hall', who:'hannah', pre:'(holding a plate she did not build)', text:'I did not ask for any of this.', expr:{hannah:'curious'}},
  {bg:'hall', who:'tala', text:'I know. You are not going to be allowed to.', expr:{tala:'smile',hannah:'smile'}},
  ...
  {bg:'hall', stage:'Then two of Tita Cora’s nephews wheel out the videoke machine, because there has never been a Simbang Gabi breakfast in this hall without one.', expr:{tita:'warm',tala:'neutral'}},
  {bg:'hall', who:'tita', pre:'(taking the microphone like it has always been hers)', tl:'Ako muna.', text:'Me first.', expr:{tita:'warm'}},
  {bg:'hall', stage:'She sings something from before Tala was born. Forty people in a church basement at six in the morning know every word of it — the ones who came in late, the nephews who were pretending not to want a turn, Ma.', expr:{tita:'talk',ma:'warm',tala:'smile'}},
  {bg:'hall', stage: () => STATE.invitedHannah
     ? 'Hannah does not know one word of it and sings anyway. Ando takes the microphone next and turns out, with no warning at all, to be very good.'
     : 'Ando takes the microphone next and turns out, with no warning at all, to be very good.',
     expr:{tala:'smile',ma:'warm',hannah:'smile'}},
  {bg:'hall', stage:'Ma laughs with her hand over her mouth, which is the loudest she gets.', expr:{ma:'warm',tala:'smile',tita:'warm'},
     aside:'This is the part Tala never explains to anybody at school. Not the church at four in the morning. <em>This.</em>'},
  {bg:'hall', show:{'hall-basket':true},
     stage:'Then Tita Cora turns the machine down and passes a basket around for Lola’s hospital bill. She did not ask the family first. The whole block puts money in, and Ma has to stand there and let them.', expr:{ma:'held',tita:'warm',tala:'quiet',hannah:'neutral'}},
  ...
  {hud:['December 24','Ninth morning · Outside'], bg:'dawn', show:{'dawn-walk':false,'dawn-tala':true},
     ...
     aside: () => { const n = STATE.seen.length;
       return n >= 12 ? 'She knows this block now. The eleven parols, the notebook under her family’s name, the tray that comes out at five. Two years, and at some point, without deciding to, she started keeping track of it.'
            : n >= 5  ? 'She has started noticing things here. Not on purpose. It just happens to you after a while.'
            :           'Two years, and she still walks past most of it with her head down. There is a version of this where she never looks up at all.'; }},
...
function render(){
  ...
  showBg(b.bg);
  if(b.zoom) pushIn(b.zoom[0], b.zoom[1], b.zoom[2]); else pullOut();
  if(b.tip){ const t = $('dragtip'); t.innerHTML = txt(b.tip); t.hidden = false; }
  ...
  if(b.choice){ mode = 'choice'; dlg.hidden = true; renderChoice(txt(b.choice)); return; }
...
function endCard(){ mode = 'end'; dlg.hidden = true; $('dim').classList.add('on');
  ...
  const n = STATE.seen.length, total = Object.values(HOT).reduce((a, l) => a + l.length, 0);
  const noticed = n >= 12 ? 'She was paying attention.' : n >= 5 ? 'She noticed some of it.' : 'She kept her head down.';
...
$('hot').addEventListener('click', ev => { const c = ev.target.closest('circle[data-t]'); if(c && mode === 'say' && !looking){ ev.stopPropagation(); look(c.dataset.t, c.dataset.k); } });
